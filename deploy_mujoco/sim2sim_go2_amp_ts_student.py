...
Go2_AMP_Ts_Student_Cfg_Yu = load_cfg(
    "legged_gym/envs/Go2_AMP_Ts/Go2_AMP_Ts_Student_Config.py",
    "Go2_AMP_Ts_Student_Cfg_Yu",
)
import math
import numpy as np
...
def run_mujoco(policy, cfg):
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        ...
        # AMP_TS student 为 LSTM 网络：输入单帧 45 维 obs，h/c 状态由 jit policy 内部维护
        hist_obs = deque()
        for _ in range(2):
        ...
        for _ in range(int(cfg.sim_config.sim_duration / cfg.sim_config.dt)):
            ...
            if count_lowlevel % cfg.sim_config.decimation == 0:
                ...
                # policy 输入 = 当前帧 obs（LSTM h/c 状态在 jit policy 内部）
                policy_input[0, :] = hist_obs[-1][0, :]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--load_model', type=str, default=f'{LEGGED_GYM_ROOT_DIR}/logs/go2_amp_ts/exported/policies/policy_amp_ts.pt',help='Run to load from.')
    parser.add_argument('--terrain', action='store_true', help='terrain or plane')
    args = parser.parse_args()

    class Sim2simCfg(Go2_AMP_Ts_Student_Cfg_Yu):
        class sim_config:
            # 平地用 scene.xml；楼梯场景可换 cross_stairs.xml（在 resources/robots/go2/go2/ 下）
